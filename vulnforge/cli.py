"""
VulnForge CLI — Command Line Interface

Provides terminal-based access to VulnForge functionality.

Usage:
    vulnforge --web                    # Start web UI
    vulnforge scan -t 192.168.1.100    # CLI-only scan
    vulnforge --update-db              # Update CVE database
    vulnforge report --format json     # Generate report
"""
...
def main(ctx, web, host, port, update_db, version):
    ...
    if version:
        console.print("[bold cyan]VulnForge[/] v1.0.0")
        return
...
def scan(target, scan_type, ports, modules, output, report_format):
    ...
    from vulnforge.modules.ssh import SSHModule
    from vulnforge.modules.services import ServicesModule
    from vulnforge.modules.kernel import KernelModule
    from vulnforge.modules.credentials import CredentialsModule
    from vulnforge.modules.docker import DockerModule
    from vulnforge.modules.nfs import NFSModule
    from vulnforge.modules.samba import SambaModule
    from vulnforge.modules.sensitive_files import SensitiveFilesModule
    from vulnforge.modules.suid import SUIDModule
    from vulnforge.modules.permissions import PermissionsModule
    from vulnforge.modules.cron import CronModule
    from vulnforge.reporting.generators import JSONReporter, HTMLReporter

    console.print(Panel.fit(
        f"[bold cyan]🛡️ VulnForge[/] — Scanning [bold]{target}[/]",
        border_style="cyan"
    ))
...
def _start_web(host: str, port: int) -> None:
    ...
    try:
        from vulnforge.app import run_server
...
def _update_database() -> None:
    ...
    from vulnforge.cve.database import CVEDatabase
    from vulnforge.cve.updater import NVDUpdater
    ...
    console.print(Panel.fit(
        "[bold cyan]🛡️ VulnForge[/] — Updating CVE Database",
        border_style="cyan"
    ))
