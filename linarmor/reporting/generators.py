"""
LinArmor JSON Report Generator
...
from linarmor.config import REPORTS_DIR
from linarmor.core.target import Target
...
class JSONReporter:
    ...
    def generate(self, target: Target, output_path: Optional[Path] = None) -> Path:
        ...
        if output_path is None:
            ...
            output_path = REPORTS_DIR / f"linarmor_report_{target.ip}_{timestamp}.json"
...
class HTMLReporter:
    ...
    def generate(self, target: Target, output_path: Optional[Path] = None) -> Path:
        ...
        if output_path is None:
            REPORTS_DIR.mkdir(parents=True, exist_ok=True)
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_path = REPORTS_DIR / f"linarmor_report_{target.ip}_{timestamp}.html"
...
<head>
    ...
    <title>LinArmor Report — {target.ip}</title>
    <style>
...
<body>
    <div class="container">
        <h1>🛡️ LinArmor Security Report</h1>
        <p class="meta">
        ...
        <div class="footer">
            Generated by LinArmor v1.0.0 — Linux Security Misconfiguration Discovery Framework
        </div>
    </div>
</body>
</html>"""
